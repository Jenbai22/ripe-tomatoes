...
from models.reviews import ReviewsOut, ReviewIn, ReviewOut
...
def get_reviews_by_imdb(imdb: str, queries: ReviewQueries = Depends()):
    return {"reviews": queries.get_reviews_by_imdb(imdb)}


@router.get("/reviews", response_model=ReviewsOut)
async def get_reviews(queries: ReviewQueries = Depends()):
    return {
...
def update_review(
    ...
    if review_in.dict()["username"] == user_data["username"]:
        ...
        if review is None:
            raise HTTPException(
                status_code=404, detail="No Such Review Exists"
            )
        else:
            return review
    else:
